...
async function main() {
  ...
  try {
    await sendEmail({
      to: email,
      subject: "🧪 Prueba de Maquetado (Ancho Completo) - ZenithNexus",
      variables: {
        tipo: "test",
        contenido: `
          ...
          <div style="text-align:center;margin:24px 0;">
            <a href="https://<domain>" target="_blank" style="display:inline-block;padding:12px 24px;background-color:#6366f1;color:#ffffff;border-radius:12px;font-weight:600;text-decoration:none;">Ir a ZenithNexus</a>
          </div>
        `
      }
    });
